...
Fixpoint count (v : nat) (s : bag) : nat
  ...
  := match s with 
     | [] => 0
     | n :: l => if (v =? n) then S( count v l ) else count v l
     end.
...
Fixpoint member (v : nat) (s : bag) : bool
  ...
  := match s with 
     | [] => false
     | n :: l => if (n =? v) then true else member v l
     end. 
...
Fixpoint remove_one (v : nat) (s : bag) : bag
  ...
  := match s with 
     | [] => []
     | n :: q => if (n =? v) then q else n :: remove_one v q
     end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag
  ...
  := match s with
     | [] => []
     | n :: q => if (n =? v) then remove_all v q else n :: remove_all v q
     end.
...
Fixpoint eq_bag (s1 : bag ) (s2 : bag) : bool 
  := match s1, s2 with 
     | [], [] => true 
     | n1 :: l1, n2 :: l2 => if (n1 =? n2) then eq_bag l1 l2 else false
     | _, _ => false
     end.
...
Fixpoint included (s1 : bag) (s2 : bag) : bool
  ...
  := match s1 with
     | [] => true
     | s :: l => if (eq_bag (remove_one s s2) s2) then false else included l (remove_one s s2)
     end.
...
Theorem add_inc_count (s : bag) (n : nat) : S( count n s) = ( count n (n::s)). 
Proof.
  Admitted. 
...
Theorem rev_aplus_cons (l : natlist) (n : nat):
  n :: l  = [n] ++ l.
Proof. 
  reflexivity. Qed.
...
Theorem hp_eq_ref1: forall n : nat,
   (n =? n) = true.
Proof. 
  intros n.
  induction n as [| n' IH]. 
...
Proof.
  ...
  induction l.
  - reflexivity. 
  -  destruct n. rewrite IHl. reflexivity. rewrite IHl. simpl.
    destruct n. reflexivity. rewrite hp_eq_ref1. reflexivity.
Qed. 
(** [] *)
...
Proof.
  ...
  - (* S n' *)
    ...
    Theorem s_helper : forall (s : bag),
      (count 0 s <=? S (count 0 s)) = true.
    Proof. 
      intro s.
      apply  leb_n_Sn. 
    Qed. 
...
Proof.
  (* FILL IN HERE *) intro s. 
  induction s. 
  - reflexivity.
  - destruct n. simpl. apply s_helper. apply IHs. Qed.    
(** [] *)
...
Proof.
  (* FILL IN HERE *) intro x. induction x. 
