...
[[ T; [T1 | T2 | ... | Tn] ]]
    ...
    the second subgoal, etc. So [T; T'] is equivalent to locally
    performing [T'] on all the subgoals:
...
(** The local form can also be used with a [..] in the middle of the list of
    tactics:

[[ T; [T1 .. | Tn] ]]

    That causes [T1] to be applied to the first goal, [Tn] to be applied to the
    last goal, and [T1] also to be applied to all the goals between the first
    and last. *)

(** Note that there is no bar between [T1] and [..]. The bar can be supplied,
    though:

[[ T; [T1 | .. | Tn] ]]

    Then no tactic is applied to the goals between the first and the last.

    Other tactics can appear in the list before or after the [..], or can be
    omitted:
    ...
    Of course only one [..] can appear in the list, becuase otherwise it would be
    ambiguous how many times to repeat the tactic. *)

(** As an example, here is a lemma we proved in [IndProp]: *)
...
(** Using [..], we can shorten it as follows: *)
